...
#endif
{
   ...
   if (strstr(game_dir, "/") != NULL){
	   // so we don't have a hl >= 3.1.1.1 here
   ...
   else if (strcmpi(mod_name, "ns") == 0)
   {
      g_pGame = new CGameCS;
#ifndef __linux__
      strcpy(game_dll_filename, "ns\\dlls\\ns.dll");
#else
      strcpy(game_dll_filename, "ns/dlls/ns_i386.so");
#endif
   }
