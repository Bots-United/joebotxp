...
CEntity UTIL_FindEntityByClassname(CEntity pStartEntity, const char *szName );
CEntity UTIL_FindEntityByTargetname(CEntity pStartEntity, const char *szName );
CEntity UTIL_FindEntityGeneric(const char *szName, Vector &vecSrc, float flRadius );
...
//bool FVisibleEx(Vector v_start, Vector v_end, bool bignore_doors=false, bool bignore_breakables=false, bool bignore_monsters=true, bool bignore_glass=true,edict_t *pEdictIgnore=0);
...
//void UTIL_PrintMessage(const char *sz1,const char *sz2=0,const char *sz3=0,const char *sz4=0);
...
edict_t *UTIL_GetNearestPlayer(edict_t *pEdict,int iTeam,float &fMin,bool bVisible=false,bool bIVC=false,float fMax=1000000);

long UTIL_CountLines(const char *szString);

void BotDebug(entvars_t *client,Vector &V,const char *,const char *,const char *,const char *,int,int);

void UTIL_ShortenBV(char *);

// a fuckin class for the fucking for_each alg.
template <class T> 
class Destroy{
	public:
		void operator()(T p){
